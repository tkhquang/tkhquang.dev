...
export default function Drawer({
  ...
  /* The slide in flight, so whichever slide starts next takes over from
     wherever it had got to instead of being dropped */
  ...
  const animateOut = useCallback(() => {
    ...
    /* A close asked for mid-slide takes over from wherever the slide is.
       Returning here while the slide in was still running, with nothing
       to retry the close, is what left a drawer standing open: the open
       state had already flipped, so no later press on the backdrop, the
       dismiss or Escape had anything left to change. */
    timelineRef.current?.kill();
    isAnimatingRef.current = true;
    const transforms = getTransformValues(position, size);
      ...
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          /* A close asked for inside these two frames has already played
             out below; sliding in now would stand the drawer back up */
